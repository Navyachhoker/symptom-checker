...
import { sendMessage, fetchHistory, fetchSessions, getUserToken } from "../api";
...
export default function Chat() {
  ...
  useEffect(() => {
    loadSessions();
    setTimeout(() => setUserToken(getUserToken()), 1000);
  }, []);
  ...
  async function selectSession(id) {
    ...
    try {
      ...
      if (r.data.triage_outcome) {
        setOutcome(r.data.triage_outcome);
        setDone(true);
      }
    } catch {}
  }
  ...
  async function send() {
    ...
    try {
      ...
      if (d.agent_trace)    setAgentTrace(d.agent_trace);
  ...
  return (
    <div style={{
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {userToken && (
              <div style={{
                ...
                title="Your anonymous session token"
              >
                🔒 {userToken.slice(0, 8)}
              </div>
            )}
        ...
        {/* Triage result with trace */}
        {outcome && (
          <TriageResult outcome={outcome} trace={agentTrace} />
        )}
